...
- Make monetary donations through categorized giving options with hosted payment processing
...
**Dual payment gateway integration with hybrid native-WebView detection.** The app runs two separate payment flows for different use cases. The shop module integrates a native payment SDK, building checkout options from server-returned order data and handling both success and structured error payloads through the same API middleware pipeline. The donations module uses a hosted gateway, which redirects to an external hosted payment page inside a WebView. Since the hosted gateway's encrypted response arrives through form submissions or URL redirects within the WebView, the app injects JavaScript that scans the DOM and URL query parameters for the encrypted payload, posts it back to React Native via `postMessage`, and handles the full lifecycle — including external bank redirects via `Linking.openURL` — without exposing the payment flow to the user as a browser escape hatch. Both paths converge on a common confirmation pattern: payment result data is structured into a backend payload and dispatched through the same API contract.
...
- Notable integrations: ERP backend, native payment SDK, hosted payment gateway via WebView bridge, push messaging with Notifee foreground notifications, background audio service
